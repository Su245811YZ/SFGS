...
for capture_path in capture_path_list:
    ...
    for frame_idx in frame_idxs:
        ...
        results['psnr'].append(psnr(out, gt).cpu().item())
        results['ssim'].append(ssim(out, gt).cpu().item())
        results['lpips'].append(lpips(out*2-1, gt*2-1).cpu().item())
...
# 保存为 CSV 文件
df.to_csv('best28.csv', index=False)  # index=False 表示不保存行索引

print("保存成功：results.csv")
