...
class Trainer(Base):
    ...
    def __init__(self):
        ...
        self.writer = SummaryWriter(log_dir=os.path.join(cfg.model_dir, 'tb_logs'))
    ...
    def load_model(self):
        ...
        cur_epoch = max([
            int(file_name[file_name.find('snapshot_') + 9: file_name.find('.pth')])
...
class Tester(Base):
    ...
    def _make_model(self):
        ...
        print(">>> Parameters saved in optimizer state:")
        for pid in opt_state.keys():
            found = False
            for param in model.module.parameters():
                if id(param) == pid:
                    print(f"- {id_to_name[pid]} (id={pid})")
                    found = True
                    break
            if not found:
                print(f"- UNKNOWN PARAM ID {pid}")
