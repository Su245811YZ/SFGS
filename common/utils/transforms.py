...
def get_view_matrix(R, t):
    ...
    view_matrix = torch.cat((Rt, torch.FloatTensor([0,0,0,1]).to(device).view(1,4)))
    return view_matrix
...
def get_fov(focal, princpt, img_shape):
    ...
    fov = torch.FloatTensor([fov_x, fov_y]).to(device)
    return fov

def get_covariance_matrix(scale, rotation):
    point_num = scale.shape[0]
    S = torch.zeros((point_num, 3, 3)).float().to(device)
    S[:,0,0] = scale[:,0]
    S[:,1,1] = scale[:,1]
    S[:,2,2] = scale[:,2]
