...
def main():
    ...
    render_shape = cv2.imread('/home/suyuze/workspace/ExAvatar_mine/data/XHumans/data/tvxq_mirotic/smplx_optimized/renders/0_smplx.jpg').shape[:2]
    video_out = cv2.VideoWriter(motion_name + '1.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 30, (render_shape[1]*4, render_shape[0]))
    ...
    for frame_idx in tqdm(frame_idx_list[0:500]):
        ...
        # forward
        with torch.no_grad():
            human_asset, human_asset_refined, human_offset, mesh_neutral_pose = tester.model.module.human_gaussian(smplx_param, cam_param,True,t)
        ...
        # print(img0.shape, img.shape, mesh_render.shape, render.shape)
        img = resize_like(img, (H, W))
        mesh_render = resize_like(mesh_render, (H, W))
        # render = resize_like(render, (H, W))
