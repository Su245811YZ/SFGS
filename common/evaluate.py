...
import re
def compute_sided_distance_scipy(points_src, points_tgt):
    """
    使用 KDTree 在 CPU 上计算每个 points_src 中的点到 points_tgt 的最小距离和索引
    输入：
        points_src: (N, 3) torch.Tensor or np.ndarray
        points_tgt: (M, 3) torch.Tensor or np.ndarray
    输出：
        distances: (N,) np.ndarray，每个点的最小距离
        indices:   (N,) np.ndarray，每个点最近邻在 points_tgt 中的索引
    """
    if isinstance(points_src, torch.Tensor):
        points_src = points_src.detach().cpu().numpy()
    if isinstance(points_tgt, torch.Tensor):
        points_tgt = points_tgt.detach().cpu().numpy()

    # print("shape(points_tgt):", np.shape(points_tgt))
    # print("points_tgt ndim:", np.ndim(points_tgt))

    points_tgt = np.squeeze(points_tgt, axis=0)
    tree = cKDTree(points_tgt)
    distances, indices = tree.query(points_src, k=1)
...
def compute_iou_w_points(points, gt_points):
    if points.shape[0] == 0 or gt_points.shape[0] == 0:
        print("Warning: One or both point clouds are empty!")
        return 0.0  # 可以选择返回 0，表示没有重叠
    ...
    vol1 = (mesh_bounds[1, 0] - mesh_bounds[0, 0]) * (
        mesh_bounds[1, 1] - mesh_bounds[0, 1]) * (mesh_bounds[1, 2] - mesh_bounds[0, 2])
    vol2 = (gt_mesh_bounds[1, 0] - gt_mesh_bounds[0, 0]) * (
        gt_mesh_bounds[1, 1] - gt_mesh_bounds[0, 1]) * (gt_mesh_bounds[1, 2] - gt_mesh_bounds[0, 2])
...
def find_hand_face_points(points_src, normals_src, points_smplx,
    ...
    _, close_id_src = compute_sided_distance_scipy(points_src, points_smplx)
...
def evaluate_per_frame(mesh_src,
    ...
    print("rhand_points_src.shape")
    print(rhand_points_src.shape)
    print("lhand_points_src.shape")
    print(lhand_points_src.shape)
...
def evaluate(pd_dir=None, gt_dir=None, gender=None, mode='test'):
    with open(osp.join('/home/suyuze/workspace/ExAvatar_mine/common/utils/human_model_files/smplx/MANO_SMPLX_vertex_ids.pkl'), 'rb') as f:
        verts_ids = pickle.load(f, encoding='latin1')
    # verts_ids = pkl.load(open(f'/home/suyuze/workspace/ExAvatar_mine/common/utils/human_model_files/smplx/MANO_SMPLX_vertex_ids.pkl', 'rb'), encoding='latin1')
    lhand_ids = torch.tensor(verts_ids['left_hand']).cuda()
    rhand_ids = torch.tensor(verts_ids['right_hand']).cuda()
    face_ids = np.load('/home/suyuze/workspace/ExAvatar_mine/common/utils/human_model_files/smplx/SMPL-X__FLAME_vertex_ids.npy')

    # pd_data_list = sorted(glob.glob(os.path.join(pd_dir, f'meshes_{mode}', '*.ply')))
    
    pd_data_list = (
        sorted(glob.glob('/home/suyuze/workspace/ExAvatar_xhumans/outputGT/result/00028/test/Take6/*.ply')) +
        sorted(glob.glob('/home/suyuze/workspace/ExAvatar_xhumans/outputGT/result/00028/test/Take8/*.ply')) +
        sorted(glob.glob('/home/suyuze/workspace/ExAvatar_xhumans/outputGT/result/00028/test/Take14/*.ply')) 
        
    )

    
    mesh_dir = '/home/suyuze/workspace/ExAvatar_mine/data/XHumans/data/00028/mesh'
    files = glob.glob(os.path.join(mesh_dir, '*.ply'))

    # 提取 take 编号和 frame 编号用于排序
    def sort_key(path):
        filename = os.path.basename(path)
        take_match = re.search(r'Take(\d+)', filename)
        frame_match = re.search(r'f(\d+)', filename)

        take_num = int(take_match.group(1)) if take_match else 0
        frame_num = int(frame_match.group(1)) if frame_match else 0

        return (take_num, frame_num)  # 先按 Take 编号，再按帧编号排序

    # 正确排序
    gt_data_list = sorted(files, key=sort_key)

    
    # gt_data_list = sorted(glob.glob('/home/suyuze/workspace/ExAvatar_mine/data/XHumans/data/00028/mesh/*.*'))[:3]
    # smplx_data_list = sorted(glob.glob(os.path.join(gt_dir, mode, '*', 'SMPLX', '*.ply')))
    smplx_data_list = (
    sorted(glob.glob('/home/suyuze/workspace/ExAvatar_mine/data/XHumans/data/00028/test/Take6/SMPLX/*.ply')) +
    sorted(glob.glob('/home/suyuze/workspace/ExAvatar_mine/data/XHumans/data/00028/test/Take8/SMPLX/*.ply')) +
    sorted(glob.glob('/home/suyuze/workspace/ExAvatar_mine/data/XHumans/data/00028/test/Take14/SMPLX/*.ply')) 
    )
    
    print(pd_data_list[:3])
    print(gt_data_list[:3])
    print(smplx_data_list[:3])
    ...
    for pd_data, gt_data, smplx_data in tqdm(
        ...
        pd = trimesh.load(pd_data)
        gt_mesh = trimesh.load(gt_data)
        smplx_mesh = trimesh.load(smplx_data)
        faces = smpl_x.face_upsampled  # (F, 3)
        if isinstance(faces, torch.Tensor):
            faces = faces.cpu().numpy()
        vertices = pd.vertices 
        pd_mesh = trimesh.Trimesh(vertices=vertices, faces=faces, process=False)
        # 打印 shape
        # print("before:", pd_mesh.vertices.shape, pd_mesh.faces.shape)
        # # 创建旋转矩阵：绕 x 轴 -90 度（顺时针）
        # rotation_matrix = trimesh.transformations.rotation_matrix(
        #     angle=np.radians(-90),
        #     direction=[1, 0, 0],   # x轴
        #     point=[0, 0, 0]        # 以原点旋转
        # )
        # pd_mesh.apply_transform(rotation_matrix)
    ...
    np.savez(os.path.join('/home/suyuze/workspace/ExAvatar_mine/0723bestall/result/00087', '_metrics.npz'),
